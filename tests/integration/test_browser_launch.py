...
def test_browser_launch():
    ...
    chrome_profile = settings.get('browser_profile_path', '') or settings.get('chrome_profile_path', '')

    if not chrome_profile:
        print("エラー: Chromeプロファイルパスが設定されていません")
        print("config/settings.json を編集して browser_profile_path を設定してください")
        print("\nプロファイルパスの例:")
    ...
    # Chrome競合チェック
    print("\nChrome競合チェック...")
    is_safe, msg = check_chrome_conflict(chrome_profile)
    if not is_safe:
        print(f"警告: {msg}")
        return False
    print("✓ Chrome競合なし")
    ...
    try:
        ...
        if page:
            page.close()
        # close_browser_contextは引数を取らない（グローバル変数を使用）
        close_browser_context()
        print("✓ クリーンアップ完了")
...
def test_chrome_conflict_detection():
    ...
    # ダミーパスでチェック
    settings = load_settings()
    profile_path = settings.get('browser_profile_path', 'Default')

    is_safe, msg = check_chrome_conflict(profile_path)
    ...
    if not is_safe:
        ...
        print("検出結果: Chrome は起動していません (競合なし)")

    print(f"\n✓ check_chrome_conflict() = {is_safe}, {msg}")
    return True
