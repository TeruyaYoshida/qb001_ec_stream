...
| 項目 | 説明 | 例 |
|------|------|-----|
| **ブラウザプロファイル** | 自動管理（変更不可） | `/Users/xxx/data/chrome_profile` |
...
本システムは専用のChromeプロファイルを使用するため、初回のみ手動でのログインが必要です。

1. 設定画面の「**ブラウザ認証設定を開く**」ボタンをクリック
2. 専用ブラウザが起動し、以下の3つのタブが自動で開く
   - Yahoo! JAPAN ログインページ
   - 佐川スマートクラブ ログインページ
   - Googleアカウント ログインページ
3. 各タブでログインを行う
   - **重要**: 「ログインしたままにする」等のオプションにチェックを入れてください
4. 全てのログインが完了したら、ブラウザを閉じる
5. アプリ画面に「認証設定が完了しました」と表示される

### 手順4: Gmail API認証

1. 「出品」ボタンをクリック
2. ブラウザが開き、Googleログイン画面が表示される（Gmail API用）
...
| Chromeが起動しています | その他のエラー | 一時的なエラーの可能性があります。再実行してください |
...
1. ヤフオクで商品が落札され、**支払いが完了**していることを確認
...
| e飛伝Webにログインしていません | セッション切れ | 設定画面の「ブラウザ認証設定を開く」から再ログインしてください |
...
1. 設定画面を開く
2. 「ブラウザ認証設定を開く」ボタンをクリック
3. 開いたブラウザでヤフオクにログインし直す
4. ブラウザを閉じて再実行
