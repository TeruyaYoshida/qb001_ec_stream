...
Gmailとの連携は、Google推奨のセキュアな認証方式「OAuth 2.0（Gmail API）」を利用します。そのため、ID・パスワードによる直接的なログイン管理とは扱いが異なります。

1.  **パスワードの共有は一切不要**:
    *   API連携では、認可後に発行される「アクセストークン」を使用します。
    *   そのため、貴社の**Googleアカウントのパスワードを弊社へ共有いただく必要はございません**。

2.  **検証の進め方**:
    *   開発および基本動作の確認は、弊社が用意するテスト用Googleアカウントにて行います。
    *   貴社環境での接続テストは、納品時（またはリモート接続時）に、お客様ご自身で「連携認証ボタン」を一度クリックしていただくのみで完了します。

3.  **トークン情報の管理**:
    *   認証後に生成される接続用ファイル（`token.json`）は、**銀行のキャッシュカード**と同様に重要な情報です。
    *   これはお客様のPC内（ローカル環境）にのみ保存され、外部サーバへ送信されることはありません。
